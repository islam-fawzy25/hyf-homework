...
function getReply(command) {
  ...
  if (commandLowerCase.startsWith('what') && command.endsWith('name?')) {
    return ' Your name is ' + nameArr[0];
  };

  //my todo part : 
  // if statment for adding 
  if (commandLowerCase.startsWith('add') && command.endsWith('todo')) {
  ...
  // removing part:
  if (commandLowerCase.startsWith('remove') && command.endsWith('todo')) {
    splitCommand.splice(0, 1);
    splitCommand.splice(splitCommand.length - 3, 3);
  ...
  // return in my todo
  if (commandLowerCase === 'what  is on my todo?') {
    return 'You have ' + myTodo.length + ' todos ' + myTodo
  };

  // here implement date of today 
  if (commandLowerCase === 'what day is it today?') {
